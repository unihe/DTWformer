...
from layers.sdtw_cuda_loss import SoftDTW
...
class Transformer_Layer(nn.Module):
    ...
    def update_epoch(self, epoch):
        ...
        #print(f"multi*********** {self.current_epoch}")

    def forward(self, x):
        self.inter_patch_attention.update_epoch(self.current_epoch)
        new_x  = x
        batch_size = x.size(0)
...
class Inter_Patch_Attention(nn.Module):
    ...
    def forward(self, Q, K=None, V=None, prev=None, key_padding_mask=None, attn_mask=None):
        self.sdp_attn.update_epoch(self.current_epoch)
...
class ScaledDotProductAttention(nn.Module):
    ...
    def update_epoch(self, epoch):
        self.current_epoch = epoch
        #print(f"最终*********** {self.current_epoch}")

class yScaledDotProductAttention(nn.Module):
    r"""Scaled Dot-Product Attention module (Attention is all you need by Vaswani et al., 2017) with optional residual attention from previous layer
    (Realformer: Transformer likes residual attention by He et al, 2020) and locality self sttention (Vision Transformer for Small-Size Datasets
    by Lee et al, 2021)"""
    ...
    def __init__(self, d_model, n_heads, attn_dropout=0., res_attention=False, lsa=False):
        ...
        self.lsa = lsa
        self.current_epoch =0
        self.sdtw = SoftDTW(use_cuda=True, gamma=0.7)
    ...
    def forward(self, q, k, v, prev=None, key_padding_mask=None, attn_mask=None):
        ...
        if self.current_epoch == 0:
            ...
            # 第一个 epoch 使用 SoftDTW
            bs, n_heads, max_q_len, d_k = q.size()
            _, _, d_k, seq_len = k.size()
            q = q.cuda()
            k = k.cuda()
            v = v.cuda()
            # 计算每个批次中Q和K的DTW距离
            attn_scores = torch.zeros((bs, n_heads, max_q_len, seq_len), device=q.device)
            for b in range(bs):
                for h in range(n_heads):
                    q_s = q[b, h].unsqueeze(0)
                    k_s = k[b, h].transpose(1, 0).unsqueeze(0)  # 将K转置以匹配SoftDTW的输入形状
                    attn_scores[b, h] = self.sdtw(q_s, k_s).squeeze(0)
...
class aaScaledDotProductAttention(nn.Module):
    r"""Scaled Dot-Product Attention module (Attention is all you need by Vaswani et al., 2017) with optional residual attention from previous layer
    (Realformer: Transformer likes residual attention by He et al, 2020) and locality self sttention (Vision Transformer for Small-Size Datasets
    by Lee et al, 2021)"""
    ...
    def __init__(self, d_model, n_heads, attn_dropout=0., res_attention=False, lsa=False):
        ...
        self.attention_saved = None  # 用于保存第一次训练时的注意力分数
        self.training_done = False  # 用于指示是否完成了第一次训练

    def forward(self, q, k, v, prev=None, key_padding_mask=None, attn_mask=None):

        # Scaled MatMul (q, k) - similarity scores for all pairs of positions in an input sequence
        if self.training_done and self.attention_saved is not None:
            # 在第一次训练之后，直接使用保存的注意力分数
            return torch.matmul(self.attention_saved, v)
        #attn_scores = torch.matmul(q, k) * self.scale  # attn_scores : [bs x n_heads x max_q_len x q_len]
    ...
    def freeze_parameters(self):
        ...
        self.training_done = True  # 指示训练已完成
